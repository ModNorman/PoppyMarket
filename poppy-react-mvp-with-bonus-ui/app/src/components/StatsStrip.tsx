...
export default function StatsStrip({ stats, loading, tooltips }: Props){
  const cells = [
  ...
  return (
    <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-7">
      {cells.map(c=> (
        <div key={c.key} className="stat bg-gradient-to-br from-rose-50 via-white to-stone-50" title={tooltips?.[c.key] || ''}>
          <div className="text-stone-500 text-sm mb-1">{c.label}</div>
          {loading ? <div className="h-6 w-24 animate-pulse rounded bg-stone-200"/> : (
            <div className="font-semibold">{
              typeof stats?.[c.key] === 'number' ? stats?.[c.key] : 0
            }</div>
          )}
        </div>
      ))}
